...
def _sarima_forecast(history: list[float], horizon: int) -> list[float]:
    try:
        from statsmodels.tsa.statespace.sarimax import SARIMAX
        if len(history) < 10:
            return _baseline(history, horizon)
        
        model = SARIMAX(history, order=(1, 1, 1), seasonal_order=(1, 0, 1, 7), enforce_stationarity=False, enforce_invertibility=False)
...
def predict_demand(history: list[float], horizon: int = 3) -> list[float]:
    ...
    baseline = _sarima_forecast(clean, horizon)
